...
static const Rat R0{ZERO, ONE};  // 常量 0/1
static void trim(Big& a) {  // 去掉高位的 0，并把零的符号规范为正
...
Big pow10(size_t k) {
    Big a; a.d.assign(k + 1, 0);
    a.d[k] = 1;
    return a;
}
Big fromStr(const std::string& s) {  // 字符串 → 大整数
    Big a; size_t i = 0;
    if (i < s.size() && (s[i] == '+' || s[i] == '-')) { a.neg = (s[i] == '-'); ++i; }
    for (size_t j = s.size(); j-- > i; ) {  // 只取数字字符，其余忽略
        if (s[j] >= '0' && s[j] <= '9') a.d.push_back((unsigned char)(s[j] - '0'));
    }
    if (a.d.empty()) a.d.push_back(0);  // 空串按 0 处理
    trim(a); return a;
}
std::string toStr(const Big& a) {  // 大整数 → 字符串
    std::string s = a.neg ? "-" : "";
    for (size_t i = a.d.size(); i-- > 0; ) s += (char)('0' + a.d[i]);
    return s;
}
...
Big addAbs(const Big& a, const Big& b) {  // 逐位相加，满十进一
    Big r; int carry = 0;
    for (size_t i = 0; i < a.d.size() || i < b.d.size() || carry; i++) {
        int v = carry + (i < a.d.size() ? a.d[i] : 0) + (i < b.d.size() ? b.d[i] : 0);
        r.d.push_back((unsigned char)(v % 10)); carry = v / 10;
    }
    trim(r);
    return r;
}
Big subAbs(const Big& a, const Big& b) {  // 逐位相减，不够减时向前借一当十
    Big r; int borrow = 0;
...
Big add(const Big& a, const Big& b) {  // 同号相加，异号取绝对值较大者的符号
    if (a.neg == b.neg) {
        Big r = addAbs(a, b);
        r.neg = a.neg;
        trim(r);
...
Big sub(const Big& a, const Big& b) { return add(a, negate(b)); }
Big mul(const Big& a, const Big& b) {  // 竖式乘法，O(n^2)
...
void divmod(const Big& a, const Big& b, Big& q, Big& r) {  // 模拟长除法
    ...
    for (size_t i = A.d.size(); i-- > 0; ) {
        r.d.insert(r.d.begin(), A.d[i]);  // r = r * 10 + 当前位
        trim(r);
        unsigned char cnt = 0;
        while (cmpAbs(r, B) >= 0) {
            r = subAbs(r, B); cnt++;
...
Big isqrt(const Big& n) {  // 牛顿迭代求 floor(sqrt(n))，初值取上界
    if (isZero(n)) {
        return ZERO;
    }
    Big x = pow10((n.d.size() + 1) / 2);
    for (;;) {
        Big q, r; divmod(n, x, q, r);
        Big y = quo(add(x, q), TWO);  // y = (x + n / x) / 2
        if (cmpAbs(y, x) >= 0) {
            return x;
        }
        x = y;
...
Rat rNorm(Big p, Big q) {  // 约分，负号归到分子
    if (q.neg) {
...
Rat rFromStr(const std::string& s) {  // 整数 / 小数 / 分数 → 有理数
    size_t sl = s.find('/');
    if (sl != std::string::npos) {
        return rNorm(fromStr(s.substr(0, sl)), fromStr(s.substr(sl + 1)));
    }
    size_t dot = s.find('.');
    if (dot != std::string::npos) {  // 0.125 → 125/1000 → 1/8，不经过 double
...
std::string rStr(const Rat& a) {  // 分母为 1 时只输出分子
...
void sqrtSplit(const Big& n, Big& s, Big& r) {  // n = s^2 · r，r 无平方因子
    s = ONE; r = n;
    for (Big f = TWO; f.d.size() <= 5; f = add(f, ONE)) {  // 试探到 10^5，保证性能
        Big f2 = mul(f, f);
        if (cmpAbs(f2, r) > 0) {
            break;
        }
...
std::string oneRoot(Big A, Big B, Big C, const Big& r, bool plus) {  // (A ± B√r)/C
    if (cmpAbs(r, ONE) == 0) {
        return rStr(rNorm(plus ? add(A, B) : sub(A, B), C));  // 完全平方
    }
    Big b = plus ? B : negate(B);
    std::string rad = "sqrt(" + toStr(r) + ")";
    std::string co = cmpAbs(b, ONE) == 0 ? "" : toStr(absBig(b));  // 系数为 1 时省略
    std::string t = (b.neg ? "-" : "") + co + rad;
    if (isZero(A)) {
        return t + (cmpAbs(C, ONE) == 0 ? "" : "/" + toStr(C));  // 例如 sqrt(2)/2
    }
    t = toStr(A) + (b.neg ? " - " : " + ") + co + rad;
    return cmpAbs(C, ONE) == 0 ? t : "(" + t + ")/" + toStr(C);  // 例如 (1 + sqrt(5))/2
}
std::string oneDec(const Big& A, const Big& B, const Big& C, const Big& r, bool plus, int prec) {
    ...
    int k = prec + (int)toStr(B).size() + 2;            // 多算保护位，抵消 √r 的截断误差
    Big sc = pow10(k), S = isqrt(mul(r, mul(sc, sc)));  // S = floor(√r · 10^k)
    Big num = plus ? add(mul(A, sc), mul(B, S)) : sub(mul(A, sc), mul(B, S));
    return decStr(num, mul(C, sc), prec);
}
static void addTerm(const std::string& t, Rat& A, Rat& B, Rat& C) {  // 一项，如 "1x^2" / "-3/4x"
    if (t.empty() || t == "+" || t == "-") return;  // 单独的正负号不是一项
    size_t x = t.find('x');
    std::string cs = x == std::string::npos ? t : t.substr(0, x);  // 系数部分
    if (cs.empty() || cs == "+") {
        cs = "1";
    }
    else if (cs == "-") {
...
void parseEquation(const std::string& s, Rat& A, Rat& B, Rat& C) {
    A = B = C = R0;
    if (s.find('x') == std::string::npos) {  // 依次填入三个下划线：a b c
        std::vector<std::string> v;
        std::string tok;
        for (char ch : s + " ") {  // 末尾补空格以便收下最后一个数
            if (ch == ' ' || ch == '\t' || ch == '\r' || ch == ',') {
                if (!tok.empty()) {
                    v.push_back(tok);
                    tok.clear();
...
Solution solve(const Rat& a, const Rat& b, const Rat& c) {  // 只求解，不输出
    ...
    if (isZero(a.p)) return s;                              // a = 0，kind 保持 Nonquadratic
    s.D = rSub(rMul(b, b), rMul(ri(4), rMul(a, c)));         // D = b^2 - 4ac
    if (s.D.p.neg) { s.kind = Solution::NoReal; return s; }  // D < 0

    if (isZero(s.D.p)) {                                     // 重根 x = -b / 2a
        Rat x = rDiv(Rat{negate(b.p), b.q}, rMul(ri(2), a));
        s.kind = Solution::Double;
        s.A = x.p;
        s.B = ZERO;
        s.C = x.q;
        s.r = ONE;  // 用 B = 0、r = 1 表示有理根
        return s;
    }
    Big t, r;  // D = P/Q 时 √D = √(P·Q)/Q = t√r/Q
    sqrtSplit(mul(s.D.p, s.D.q), t, r);
    Rat nb{negate(b.p), b.q};                         // -b
    Rat num = rMul(nb, Rat{s.D.q, ONE});              // -b·Q
    Rat den = rMul(rMul(ri(2), a), Rat{s.D.q, ONE});  // 2a·Q
    Big A = mul(num.p, den.q), 
        B = mul(t, den.q), 
        C = mul(num.q, den.p);  // x = (A ± B√r)/C
    if (C.neg) {
        A = negate(A);
        B = negate(B);
        C = negate(C); 
    }
    Big g = gcd(gcd(A, B), C);  // 提取公因子，化为最简根式
...
void printSolution(const Solution& s, bool math) {  // 只输出，不计算
    ...
    if (s.kind == Solution::NoReal) { std::cout << "No real roots\n"; return; }
    int n = s.kind == Solution::Double ? 1 : 2;  // 重根只输出一行
    for (int i = 0; i < n; i++) {                // x1 = (-b + √D) / 2a
